...
public class VotesService {
    ...
    public VoteResponse getLikeResponse(final VoteRequest voteRequest, final Principal principal) {
        ...
        if (postVote != null && postVote.getValue() == 1) {
            return new VoteResponse(false);
        }
        setVote(currentUser, postRepository.findPostById(postId), postVote, (byte) 1);
    ...
    private void setVote(final User user, final Post post, final PostVote postVote, final byte value) {
        if (postVote != null) postVotesRepository.delete(postVote);
